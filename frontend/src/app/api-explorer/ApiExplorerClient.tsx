...
const ENDPOINTS: EndpointDoc[] = [
  {
    ...
    curlExample: `curl -X GET "https://api.weatherinsight.id/api/health"`,
    pythonExample: `import requests
resp = requests.get("https://api.weatherinsight.id/api/health")
data = resp.json()
print(f"Status: {data['status']}, Cities: {data['cities_count']}")`,
    jsExample: `const resp = await fetch("https://api.weatherinsight.id/api/health");
...
console.log(\`Status: \${data.status}\`);`,
  },
  {
    ...
    curlExample: `curl -X GET "https://api.weatherinsight.id/api/cities?province=Jawa"`,
    pythonExample: `import requests
resp = requests.get("https://api.weatherinsight.id/api/cities", params={"province": "Jawa"})
cities = resp.json()
for c in cities:
    print(f"{c['id']}: {c['city']} ({c['province']})")`,
    jsExample: `const resp = await fetch("https://api.weatherinsight.id/api/cities?province=Jawa");
...
cities.forEach(c => console.log(\`\${c.id}: \${c.city}\`));`,
  },
  {
    ...
    curlExample: `curl -X GET "https://api.weatherinsight.id/api/cities/11"`,
    pythonExample: `import requests
resp = requests.get("https://api.weatherinsight.id/api/cities/11")
city = resp.json()
w = city.get('latest_weather')
if w: print(f"{city['city']}: {w['temperature']}°C")`,
    jsExample: `const resp = await fetch("https://api.weatherinsight.id/api/cities/11");
...
console.log(\`\${city.city}: \${city.latest_weather?.temperature}°C\`);`,
  },
  {
    ...
    curlExample: `curl -X GET "https://api.weatherinsight.id/api/weather/current"`,
    pythonExample: `import requests
resp = requests.get("https://api.weatherinsight.id/api/weather/current")
weather = resp.json()
for w in weather:
    print(f"{w['city']}: {w['temperature']}°C — {w['weather_description']}")`,
    jsExample: `const resp = await fetch("https://api.weatherinsight.id/api/weather/current");
...
weather.forEach(w => console.log(\`\${w.city}: \${w.temperature}°C\`));`,
  },
  {
    ...
    curlExample: `curl -X GET "https://api.weatherinsight.id/api/weather/current/11"`,
    pythonExample: `import requests
resp = requests.get("https://api.weatherinsight.id/api/weather/current/11")
w = resp.json()
print(f"{w['city']}: {w['temperature']}°C")`,
    jsExample: `const resp = await fetch("https://api.weatherinsight.id/api/weather/current/11");
...
console.log(\`\${w.city}: \${w.temperature}°C\`);`,
  },
  {
    ...
    curlExample: `curl -X GET "https://api.weatherinsight.id/api/history/11?days=7"`,
    pythonExample: `import requests
resp = requests.get("https://api.weatherinsight.id/api/history/11", params={"days": 7})
history = resp.json()
print(f"{len(history)} baris data — suhu rata-rata: {sum(h['temperature'] for h in history)/len(history):.1f}°C")`,
    jsExample: `const resp = await fetch("https://api.weatherinsight.id/api/history/11?days=7");
...
console.log(\`\${history.length} baris data\`);`,
  },
  {
    ...
    curlExample: `curl -X GET "https://api.weatherinsight.id/api/forecast/11?hours=24"`,
    pythonExample: `import requests
resp = requests.get("https://api.weatherinsight.id/api/forecast/11", params={"hours": 24})
fc = resp.json()
for p in fc['forecast']:
    print(f"{p['forecast_time']}: {p['temperature']}°C")`,
    jsExample: `const resp = await fetch("https://api.weatherinsight.id/api/forecast/11?hours=24");
...
fc.forecast.forEach(p => console.log(\`\${p.forecast_time}: \${p.temperature}°C\`));`,
  },
  {
    ...
    curlExample: `curl -X GET "https://api.weatherinsight.id/api/analytics/summary"`,
    pythonExample: `import requests
resp = requests.get("https://api.weatherinsight.id/api/analytics/summary")
s = resp.json()
print(f"Rata-rata: {s['avg_temperature']}°C, Terpanas: {s['hottest_city']['city']} ({s['hottest_city']['temperature']}°C)")`,
    jsExample: `const resp = await fetch("https://api.weatherinsight.id/api/analytics/summary");
...
console.log(\`Avg: \${s.avg_temperature}°C, Hottest: \${s.hottest_city.city}\`);`,
  },
  {
    ...
    curlExample: `curl -X GET "https://api.weatherinsight.id/api/analytics/compare?cities=11,15"`,
    pythonExample: `import requests
resp = requests.get("https://api.weatherinsight.id/api/analytics/compare", params={"cities": "11,15"})
data = resp.json()
for c in data:
    print(f"{c['city']}: {c['temperature']}°C, {c['humidity']}%")`,
    jsExample: `const resp = await fetch("https://api.weatherinsight.id/api/analytics/compare?cities=11,15");
const data = await resp.json();
data.forEach(c => console.log(\`\${c.city}: \${c.temperature}°C\`));`,
  },
  {
    ...
    curlExample: `curl -X GET "https://api.weatherinsight.id/api/model-metrics"`,
    pythonExample: `import requests
resp = requests.get("https://api.weatherinsight.id/api/model-metrics")
metrics = resp.json()
for m in metrics:
    s = "✅" if m['lebih_baik_dari_baseline'] else "⚠️"
    print(f"{s} {m['city']}: MAE={m['mae']}°C (baseline: {m['mae_baseline_naif']}°C)")`,
    jsExample: `const resp = await fetch("https://api.weatherinsight.id/api/model-metrics");
    ...
    <div className="animate-fade-in">
      ...
      <div className="mt-8 text-center">
        <p className="text-xs" style={{ color: "var(--text-muted)" }}>
          ...
          <a
            href="https://api.weatherinsight.id/docs"
            target="_blank"
